...
class UserFunctionalRoomController extends Controller
{
    ...
    public function rent_room(Request $request)
    {
        ...
        $function_hall->add_service_payment = ($function_hall->microphones ||
        ...
        // Create a new Sales_Rent record with relevant data
        $sales_rent = new SalesRent();
        $sales_rent->rent_id = $function_hall->rent_id; // Assuming you have an 'id' field as primary key
        $sales_rent->userid = $userid;
        $sales_rent->sale_date = $function_hall->updated_at; // Assuming 'updated_at' holds the sale date
        $sales_rent->total_amount = $function_hall->total_payment;
        $sales_rent->status = $function_hall->status;
